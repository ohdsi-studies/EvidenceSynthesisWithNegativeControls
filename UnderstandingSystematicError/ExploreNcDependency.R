...
folder <- "UnderstandingSystematicError"
...
saveRDS(ncEstimates, file.path(folder, "ncEstimates.rds"))
...
saveRDS(ncEstimates, file.path(folder, "ncEstimatesSemanaion.rds"))
disconnect(connection)



# Compute bias distributions -----------------------------------------------------------------------
ncEstimates <- readRDS(file.path(folder, "ncEstimates.rds"))
...
ggsave(file.path(folder, "SysErrorDistUnadj.png"), width = 14, height = 5, dpi = 300)

plotFigure(tableData |> filter(analysisId == 8), plotData |> filter(analysisId == 8), "PS matched")
ggsave(file.path(folder, "SysErrorDistMatched.png"), width = 14, height = 5, dpi = 300)
...
ggsave(file.path(folder, "NcEstimatesAcrossDbsGLP1RA_SU.png"), width = 10, height = 6, dpi = 300)

plotFigure("DPP4I", "GLP1RA")
ggsave(file.path(folder, "NcEstimatesAcrossDbsDPP4I_GLP1RA.png"), width = 10, height = 6, dpi = 300)
